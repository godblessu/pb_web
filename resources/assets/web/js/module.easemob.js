...
easemob = {
                    ...
                    .toArray().then(function(items){
                        ...
                        _.forEach(uids, function(value, key){
                            if (typeof(value) == 'string'){
                                _uids = _.unionWith(uids, value.split(','));
                            } else {
                                _uids.push(parseInt(value));
                            }
                        });

                        easemob.updateUsers(_uids);
                    });
            });
        }
    ...
    updateUsers: function(id){
        var uids = $.cookie('easemob_uids') === undefined ? [] : $.cookie('easemob_uids').split(',');
                ...
                .toArray().then(function(items){
                    _.forEach(items, function(value, key) {
                        ...
                        if (value.del == 0 || value.id == easemob.cid) {
                            ...
                            var html = '<li ' + css + ' class="room_item" data-type="0" data-uid="' + to + '" data-cid="' + value.id + '" data-name="' + value.title +  '" id="chat_' + value.id + '">'
                            ...
                            if (value.id == easemob.cid) {
                                ...
                                easemob.listMes(easemob.cid, value.title);
                            } else {
                        ...
                        if (value.del == 1 && value.id == easemob.cid) {
                            window.TS.dataBase.room.where({id: value.id}).modify({
                                del: 0
                            })
                        }
                    });
